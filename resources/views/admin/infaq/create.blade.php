...
<body class="bg-gray-100 font-sans min-h-screen flex items-center justify-center">
    ...
    <div class="bg-white shadow-md rounded-lg p-8 w-full max-w-lg">
        <h1 class="text-2xl font-bold mb-6 text-center text-gray-800">Tambah Infaq</h1>
        ...
        <form action="{{ route('admin.infaq.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf

            <div>
                <label for="image" class="block text-sm font-medium text-gray-700">Banner</label>
                <input type="file" id="image" name="image" placeholder="Pilih gambar"
            ...
            <div>
                ...
                <input type="number" id="target" name="target" placeholder="Masukkan target"
                    class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm px-3 py-2 focus:ring-blue-500 focus:border-blue-500"
                    required>
                @error('target')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
