                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                            ...
                            @foreach ($qrises as $qris)
                                <div class="border rounded-lg overflow-hidden shadow-lg group relative">
                                    ...
                                    <a href="{{ asset('storage/qris/' . $qris->image) }}" target="_blank"
                                        ...
                                        <form action="{{ route('admin.qris.delete', $qris->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                        ...
                        @else
                            @if (Auth::check() && Auth::user()->role == 'admin')
                                <div class="mt-6">
                                    <h4 class="text-xl font-semibold mb-2 text-left">Upload QRIS Infaq</h4>
